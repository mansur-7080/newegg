...
export class SearchService {
      ...
      const body = {
        ...
        query: {
          bool: {
            ...
            filter: [
              { term: { status: 'active' } },
              { term: { 'availability.inStock': true } }
            ]
          }
        },
  ...
  private async generateSuggestions(query?: string): Promise<string[]> {
    ...
    try {
      ...
      return response.body.aggregations?.related_queries?.buckets?.map(
  ...
  private parseAnalytics(aggs: any): any {
    return {
      ...
      zeroResultQueries: aggs.zero_results?.queries?.buckets?.map((bucket: any) => ({
        query: bucket.key,
        count: bucket.doc_count
      })) || [],
