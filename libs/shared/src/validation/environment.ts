...
export const externalServiceEnvSchema = Joi.object({
  ...
  STRIPE_PUBLISHABLE_KEY: Joi.string().pattern(/^pk_(live|test)_/).required(),
...
export const thirdPartyEnvSchema = Joi.object({
  ...
  TWILIO_PHONE_NUMBER: Joi.string().pattern(/^\+[1-9]\d{1,14}$/).optional(),
...
export function validateEnvironment(
  ...
  try {
    ...
    if (error) {
      ...
      throw new Error(
        `Environment validation failed for ${serviceName}: ${error.message}`
      );
    }
