...
export class ProductService {
   ...
   * Search products
   */
  async searchProducts(searchParams: string | { filters: any; page: number; limit: number; sortBy: string; sortOrder: string }) {
    // Handle both string query and object parameters
    if (typeof searchParams === 'string') {
      return this._searchProductsByText(searchParams);
    } else {
      return this._searchProductsWithFilters(searchParams);
    }
  }

  async _searchProductsByText(query: string, page: number = 1, limit: number = 10) {
    try {
      const skip = (page - 1) * limit;
  ...
  async _searchProductsWithFilters(params: { filters: any; page: number; limit: number; sortBy: string; sortOrder: string }) {
    try {
      ...
      logger.error('Failed to search products with filters', { error });
      throw error;
    }
  }
  ...
  async getProductBySku(sku: string) {
    try {
      const product = await Product.findOne({ sku, isDeleted: false }).lean();
      if (!product) {
  ...
  async updateInventory(id: string, quantity: number) {
    try {
      ...
      logger.error('Failed to update inventory', { id, quantity, error });
      throw error;
    }
  }

  async checkProductAvailability(productIds: string[]) {
    return this.checkAvailability(productIds);
  }

  async createCategory(categoryData: any) {
    // This would need a Category model - for now return mock
    logger.info('Create category called', { categoryData });
    return { id: 'mock-category-id', ...categoryData };
  }
  ...
  async getCategoryBySlug(slug: string) {
    // Mock implementation - would need Category model
    logger.info('Get category by slug called', { slug });
    return { slug, name: slug.replace('-', ' ') };
  }

  async createReview(reviewData: any) {
    // Mock implementation - would need Review model
    logger.info('Create review called', { reviewData });
    return { id: 'mock-review-id', ...reviewData };
  }

  async getProductReviews(productId: string, page: number = 1, limit: number = 10) {
    // Mock implementation - would need Review model
    logger.info('Get product reviews called', { productId, page, limit });
    return {
      reviews: [],
      total: 0,
      page,
