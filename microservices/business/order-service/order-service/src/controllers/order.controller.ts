...
export class OrderController {
  ...
  static async updateCartItem(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (cartItem.products.track_inventory && cartItem.products.stock_quantity < quantity) {
        throw new AppError(400, 'Insufficient stock');
      }
...
export const orderStatusValidation = [
  body('status').optional().isIn(['pending', 'processing', 'shipped', 'delivered', 'cancelled']).withMessage('Valid status is required'),
  body('payment_status').optional().isIn(['pending', 'paid', 'failed', 'refunded']).withMessage('Valid payment status is required')
];
